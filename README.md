# Google Cloud Client Libraries for Swift - Cloud Run Admin API
...
Add `swift-google-cloud-run-v2` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-run-v2.git --from 0.1.0-preview
```

Then add `GoogleCloudRunV2` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudRunV2 <target-name> --package swift-google-cloud-run-v2
```
